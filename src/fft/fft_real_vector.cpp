...
static void radix4_real(fft_simd* X, int N, int dir);
static void radix2_real(fft_simd* X, int N, int dir);

void fft_real(fft_simd* X, int N, int dir) {
	if ((ilogb(N) % 2 == 1) && (N <= 32)) {
		radix2_real(X, N, dir);
	} else {
		radix4_real(X, N, dir);
...
static void radix2_real(fft_simd* X, int N, int dir) {
	...
	if (N2o2) {
		const int _3N2o2 = N2 + (N2 >> 1);
		fft_simd& er1 = X[_3N2o2];
		er1 = -er1;
	}
	if (N2o2 > 1) {
...
static void radix4_real(fft_simd* X, int N, int dir) {
	...
	if (N2o2 > 1) {
		...
		for (int k2 = 1; k2 < N2o2; k2++) {
			...
			const fft_simd wi2 = two * wi1 * wr1;
			const int i0 = k2;
			const int i1 = i0 + N2;
			const int i2 = i1 + N2;
			const int i3 = i2 + N2;
